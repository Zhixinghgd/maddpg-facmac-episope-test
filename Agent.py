...
class Agent:
    ...
    def update_actor(self, loss):
        ...
        if self.agent_id.startswith("adversary_") or self.agent_id.startswith("leadadversary_"):
            gradient = torch.nn.utils.clip_grad_norm_(self.actor.parameters(), 0.8)
        else:
            gradient = torch.nn.utils.clip_grad_norm_(self.actor.parameters(), 0.5)
    ...
    def update_critic(self, loss):
        ...
        loss.backward()
        gradient = 0  # 裁剪前的梯度
        gradient = torch.nn.utils.clip_grad_norm_(self.critic.parameters(), 0.5)
